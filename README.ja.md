...
recalculation: ignore             # 再計算された数式の結果は編集ではない
semantic:
...
`recalculation` は、**数式そのものは変わらないのに、キャッシュされた計算
結果だけが動いた**場合の扱いを決めます。Excel は保存のたびに再計算し、
openpyxl は逆に保存済みの答えを破棄します。どちらも編集がしたことでは
ありません。既定の `ignore` はこれを変更として扱いません(件数はレポートに
「何件を対象外にしたか」として表示されます)。**肝心のケースが緩むことは
ありません** — 数式が、その時表示していた数値で上書きされたものは
「再計算」ではなく `formula` の変更なので、従来どおり FAIL します。
保存された計算結果そのものが成果物であるブックでは、
`recalculation: strict` を指定すれば、計算結果も他の値と同じように
判定されます。
...
再計算された結果は、こちらで面倒を見ます。**数式が1文字も変わっていないのに
保存された答えだけが動いたセル**は、編集ではなく再計算です。既定の
`recalculation: ignore` はこれを変更として数えません(レポートには
`5 recalculated formula results ignored` と出て PASS します)。
そのためポリシーには、人が実際に編集するセルと、セル以外の場所にある
キャッシュだけを書けば足ります:
...
これが安全なのは、**`formula` を protect したままにしている**からです。
結果は動いてよいが、それを生む数式そのものは動かしてはいけない、という
指定になります。数式がリテラルで上書きされた場合は「再計算」ではなく
数式そのものが消えているので、従来どおり `formula` 属性として検出されます。

保存された計算結果そのものが成果物である場合(Excel で開かれることのない
相手に納品するブックなど)は、`recalculation: strict` を指定した上で、
計算結果の範囲を明示的に許可してください:
...
`recalculation: ignore` はこの向きもカバーします。数式に手が入っていない
セルから答えが消えたのは、答えが計算し直されたのと同じ種類の
「何も起きていない」変化だからです。残りは手で許可することになりますが、
値だけでは足りません。ライブラリが再現できない日付セルの `format` や、
非表示列の `layout` などです。
...
- **Excel** — `layout`(行・列の非表示とサイズ)、`protection`
  ...
  Excel のエラーチェック警告(緑の三角)の抑制を含む)。`defined_names` は
  名前定義(ブックスコープ・シートスコープの両方)を対象とし、
  **参照先だけ別の範囲に付け替えられた名前**を捕まえます。
  その名前を使う数式は見た目が変わらないため、これがないと通ります。
- **Word** — `paragraph_format` / `field` / `bookmark` /
  ...
  **他のどの属性でも説明がつかないブロック内のマークアップ**を受け止める
  ...
  これで捕まります。`tables` は Word の表に対する structural キーで、
  表の追加・削除、スタイルの変更、行列構成の変化、セル内容の書き換えを
  対象とします。
...
社印画像・和暦形式の日付・テーマ色)と、Word で作成した1つの文書を開いて、
**一切編集せずに**保存し直した結果:
...
- **直せる範囲は実機 Office に直させる。** ライブラリが壊したブックを
  ...
  上の実測では openpyxl の13件が2件まで減りました。ただし残った2件は
  **非表示列の幅と失われた拡張ブロック**で、どちらも再計算では戻せません。
  **再保存は再計算であって、復活ではありません。** 実際に消された
  ものは消えたままです。
...
`@v1` は最新の**公開済み**リリースに追従する可変タグです。PyPI への
アップロードが成功した後、publish ワークフローがリリース対象のコミットへ
このタグを強制的に移動させます。したがって `main` の先端ではなく、PyPI が
配布しているものを指します。リリースとリリースの間は `main` より後ろに
なることがあり、タグを動かした変更がまだリリースされていない時期は、
逆に最新のバージョンタグより少し先に出ることもあります。監査対象のCIでは、
他のサードパーティActionと同様に、完全なコミットSHAへの固定
